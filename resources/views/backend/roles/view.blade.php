                                    ...
                                    <div class="kt-section kt-section--first">
                                        <div class="kt-section__body">
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-lg-9 col-xl-6">
                                                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" name="name" type="text" value="{{ old('name', $detail->name) }}"  placeholder="Name" required>
                                                    @if($errors->has('name'))
                                                            ...
                                                            <div class="kt-checkbox-inline">
                                                                @foreach($subItem->permissions as $permission)
                                                                    <label class="kt-checkbox kt-checkbox--tick kt-checkbox--success">
                                                                        <input type="checkbox" name="permission[]" value="{{ $permission->name }}" {{ $detail->hasPermissionTo($permission->name) ? 'checked' : '' }}> {{ $permission->permission_label }}
                                                                        <span></span>
                                                                    </label>
                                                                @endforeach
                                                            </div>
